...
- A **Mermaid dependency graph** webview + **per-track detail** panel (issue table — capped at 50 rows with a collapsible overflow — blockers, **depends-on chips**, ordered next-up, a **Plan** affordance, and a thin **open/closed progress bar** #220) — with a focus toggle that zooms in on the selected track, and a full map scoped to the track's repo.

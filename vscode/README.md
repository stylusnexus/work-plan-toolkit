...
- A **Mermaid dependency graph** webview + **per-track detail** panel (issue table — capped at 50 rows with a collapsible overflow — blockers, **depends-on chips**, ordered next-up, a **Plan** affordance, and a thin **open/closed progress bar** #220) — with a focus toggle that zooms in on the selected track, and a full map scoped to the track's repo. The graph has **zoom / pan / fit-to-width** controls (scroll-wheel + drag, or the header buttons) and **Export as SVG / PNG** (#216), so a dense map stays navigable.

...
def sync_missing_rows(body: str, frontmatter_nums: list, issues_by_num: dict):
    """Append a canonical row for every frontmatter issue missing from the table.
    ...
    and appends a row for each missing number IN FRONTMATTER ORDER. Live
    title/assignee/status come from `issues_by_num` (a {num: gh-issue-dict}
    map); a number with no fetched data still gets a placeholder row so
    membership never silently drifts.
    ...
    for num in missing:
        ...
        title = issue.get("title", "(not fetched)")
        row_lines.append(render_issue_row(
            num, title, format_assignees(issue),
            state_to_status_label(issue.get("state")),
        ))
    return append_rows(body, table, row_lines), len(missing)

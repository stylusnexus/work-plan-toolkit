...
DESCRIPTIONS = [
    ...
    ("reconcile", "<track> | --all [--draft|--apply]",
     "Sync track frontmatter with GitHub labels (read-only on GitHub). Default label is `track/<slug>`; override per-track via `github.labels: [...]` in frontmatter. Modes: (default) interactive y/N per track; --draft preview only, never write; --apply write all proposed ADDs without prompting (useful when running through the slash-command harness where the interactive prompt isn't reachable). FLAGs are advisory under all modes.",
     "WEEKLY hygiene — pulls labeled issues into their tracks, flags un-labeled ones. Use --draft for sweep audits, --apply when you trust the diff and want it written.",
     "/work-plan reconcile --all --apply"),

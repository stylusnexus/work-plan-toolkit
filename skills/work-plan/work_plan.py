...
DESCRIPTIONS = [
    ...
    ("reconcile", "<track> | --all | --repo=<key> [--draft] [--yes]",
     "Update track MEMBERSHIP (the `github.issues` list in frontmatter) by syncing it against a GitHub label. Default label is `track/<slug>`; override per-track via `github.labels: [...]` in frontmatter. Read-only on GitHub. Add --draft to preview proposed ADDs/FLAGs without prompting or writing; add --yes to apply ADDs without prompting (non-interactive, e.g. from the VS Code extension). NOT for hand-curated tracks — see `refresh-md` if you only want to update issue state.",

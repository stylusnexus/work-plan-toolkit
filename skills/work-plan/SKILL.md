...
| `/work-plan reconcile <track> \| --all [--draft\|--apply]` | Sync track frontmatter with GitHub labels (read-only on GitHub). Default label is `track/<slug>`; override per-track via `github.labels` in frontmatter. `--draft` previews proposed ADDs/FLAGs without writing; `--apply` writes all ADDs non-interactively (use this when running through the slash-command harness where the interactive prompt isn't reachable). |

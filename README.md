...
- **Local-only writes.** The skill writes to `~/.claude/skills/work-plan/`, `~/.claude/skills/repo-activity-summary/`, `~/.claude/commands/work-plan.md`, `~/.claude/work-plan/config.yml`, and your `notes_root`. The exceptions are confined to the repo you point them at and all opt-in: the `plan-status` action flags (`--stamp` writes a status header into discovered plan docs; `--archive` `git mv`s dead plans into `archive/abandoned/`; `--issues` opens GitHub issues for partial plans via `gh` — all honor `--draft` and the two mutating actions prompt), and `plan-confirm`, which writes a `verdict_override` into one plan doc's **YAML frontmatter only** (never its body/checkboxes/manifest) and is public-repo gated. Nothing else.
...
`plan-status --json` is the viewer's **Plans view** read surface: alongside each doc's verdict it now also emits `manifest_last_touched` (the most recent commit date across the plan's declared files), `stalled`, `lie_gap`, `unchecked_items`, and `stall_days`. The staleness window honors `stall_days:` in `~/.claude/work-plan/config.yml` and a `--stall-days=<n>` flag (precedence: flag → config → default 14). The viewer consumes these to flag plans whose declared-file build has gone cold — a `partial` plan with no recent commit on its manifest ("stalled") — and plans scored shipped whose own phase checkboxes are mostly unticked ("lie-gap"). It also emits `override`: the human `verdict_override` from the doc's frontmatter (`shipped`/`partial`/`dead`, or `null`), set via `plan-confirm`. When present the CLI pins the verdict to it and forces `lie_gap` false, so a confirmed-but-unticked plan stops reading as a lie.

**The Plans view can write to plan-doc frontmatter — and only frontmatter.** Right-click a plan in the viewer → **Confirm Verdict…** (or **Clear Confirmation**) drives `plan-confirm` behind a mandatory modal that names the exact file and states the write touches only the doc's YAML frontmatter — never its prose body, checkboxes, or declared-file manifest. This is the single viewer-initiated write to a plan doc; it writes `verdict_override` and nothing else. On a public repo it additionally passes through the public-repo confirm modal above. Everything else the Plans view does (scan, acknowledge) remains read-only on git.
